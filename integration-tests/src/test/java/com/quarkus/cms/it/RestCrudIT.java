...
class RestCrudIT {
  ...
  @Test
  @Order(15)
  @DisplayName("should filter entries")
  void filterEntries() {
    String docA = createEntryReturningId(Map.of(
        "title", "High Priority", "body", "Urgent", "priority", 10, "published", true));
    String docB = createEntryReturningId(Map.of(
        "title", "Low Priority", "body", "Later", "priority", 1, "published", true));

    given().contentType("application/json").when().post("/api/" + CT_SINGULAR + "/" + docA + "/publish").then().statusCode(200);
    given().contentType("application/json").when().post("/api/" + CT_SINGULAR + "/" + docB + "/publish").then().statusCode(200);

    given()
        .queryParam("filters[priority][$eq]", "10")
        .queryParam("publicationState", "live")
        .when()
        .get("/api/" + CT_SINGULAR)
        .then()
        .statusCode(200)
        .body("data.size()", is(1))
        .body("data[0].title", is("High Priority"));
  }
  ...
  void sortEntries() {
    String docA = createEntryReturningId(Map.of(
        "title", "Zeta Entry", "body", "Last", "priority", 5, "published", true));
    String docB = createEntryReturningId(Map.of(
        "title", "Alpha Entry", "body", "First", "priority", 5, "published", true));
    ...
    given()
        ...
        .body("data[0].title", is("Alpha Entry"));
  }
  ...
  void paginateEntries() {
    ...
    given()
        ...
        .body("meta.pagination.pageCount", greaterThanOrEqualTo(2));
  }
